...
class InboundEventConsumerTest {
    ...
    fun `fans out to the matching handler and deletes the message`() {
        ...
        val queue = FakeQueue(listOf(Message(envelopeJson(EventType.PAYMENT_CONFIRMED), "rh-1")))
    ...
    fun `fans out to every handler registered for the same eventType`() {
        ...
        val queue = FakeQueue(listOf(Message(envelopeJson(EventType.EXECUTION_FINISHED), "rh-2")))
